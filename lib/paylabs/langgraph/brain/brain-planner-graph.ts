...
=== LANGUAGE RULE ===
All output fields (assistant_response, user_visible_reasoning, tier_decision_reason, plan_rationale, safe_brain_summary) MUST be in English regardless of the user's query language. If the user queries in Indonesian, Chinese, Spanish, or any other language, respond in English. Do NOT mirror the user's language.
...
- 3-4 sentences explaining what the run will do, what sources will be searched, what the user gets, and the registry/source scope.
...
- MUST mention registry checks and source access scope: "up to {max_registry_checks} registry checks and up to {max_source_accesses} source accesses included in the locked quote."
- MUST NOT use vague wording like "some checks", "additional fees may apply", or "extra charges".
- Use clear wording: "up to X registry checks", "up to Y source accesses", "included in the locked quote", "included in User Cost".
...
user_visible_reasoning: "Technical documentation, implementation guides, and real-world x402 usage examples will be collected from RSSHub feeds covering blockchain protocols and developer tooling. Sources will be ranked by authority and relevance to surface the most informative references. Up to 2 registry checks and up to 2 source accesses are included in the locked quote. Entry x402 payment covers discovery and source delivery — no creator payout for this retrieval."
...
user_visible_reasoning: "The full pipeline runs: source verification evaluates the premium content's value and creator credentials, followed by paid content unlock upon approval. Advanced evidence evaluation ensures quality before creator attribution and x402 micropayment settlement. Up to 8 registry checks and up to 6 source accesses are included in the locked quote. Entry x402 payment includes payment-decision, paid source unlock, evidence evaluation, and creator payout — all phases active."
...
user_visible_reasoning: "Multiple current sources will be gathered and cross-referenced. The route uses deeper evidence validation because the task requires current comparison, verification, and source attribution. Up to 8 registry checks and up to 6 source accesses are included in the locked quote."
    ...
    if (!result.ok) {
      ...
      return {
        ...
        brainLlmDiag: {
          ...
          response_format_json: (result.meta as Record<string, unknown>)?.response_format_json ?? null,
          maxAttempts: (result.meta as Record<string, unknown>)?.retry_count !== undefined ? ((result.meta as Record<string, unknown>).retry_count as number) + 1 : null,
        },
      };
    }
