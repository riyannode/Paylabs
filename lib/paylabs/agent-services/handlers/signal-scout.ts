...
export const signalScoutHandler: ServiceHandler = async (
  ...
  if (mergedLive.length === 0 && queryHasDomainTopic) {
    ...
    if (tavilyEnabled && tavilyKeyExists) {
      try {
        ...
        if (isTavilyEnabled()) {
          // Pick the most specific topic for Tavily query
          ...
          if (tavilyResult.candidates.length > 0) {
            return {
              ...
              data: {
                ...
                safe_signal_summary: `RSSHub returned 0 ${primaryTopic.category} sources. Tavily web search returned ${tavilyResult.candidates.length} link(s).`,
                retrieval_mode: "rsshub_empty_tavily_live",
                source_strategy: "tavily_links_only_after_rsshub_empty",
                topic_routes_count: topicResult.diagnostics.topic_routes_count,
                topic_candidates_count: topicResult.candidates.length,
              },
              safeSummary: `RSSHub returned 0 sources. Tavily found ${tavilyResult.candidates.length} link(s).`,
              settled: false,
              error: null,
            };
          }
        ...
        console.warn("[signal_scout] Tavily fallback failed", {
