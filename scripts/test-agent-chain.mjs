...
if (!HMAC_SECRET) { throw new Error("Set PAYLABS_HMAC_SECRET env var"); }
console.log("HMAC loaded:", HMAC_SECRET.length, "chars, first 8:", HMAC_SECRET.slice(0,8));
...
for (const agent of agents) {
  ...
  let r;
  try { r = JSON.parse(text); } catch { r = { error: text.slice(0, 100) }; }
  const ok = res.status === 200;
  if (!ok) allPassed = false;
  const icon = ok ? "✅" : "❌";
  const paymentResp = res.headers.get("x-payment-response");
  console.log(icon, agent.payer, "→", agent.name, "HTTP", res.status, "payment:", paymentResp, r.status || r.error?.slice(0,60));
}
