...
> **Note:** PayLabs uses the real Arc testnet and Circle's official infrastructure. There are no pre-funded demo accounts. Each reviewer creates their own wallet and obtains official test USDC from the Circle Faucet, following the same onboarding flow as every real user.
