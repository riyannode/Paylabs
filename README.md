...
**Implementation:** [Decode](https://github.com/riyannode/Paylabs/blob/main/lib/paylabs/x402/decode-batch.ts), [Link payment](https://github.com/riyannode/Paylabs/blob/main/lib/paylabs/x402/payment-links.ts), [Seller](https://github.com/riyannode/Paylabs/blob/main/lib/paylabs/x402/seller-challenge.ts), [Buyer](https://github.com/riyannode/Paylabs/blob/main/lib/paylabs/x402/buyer-transport.ts), [Resolver](https://github.com/riyannode/Paylabs/tree/main/app/api/paylabs/x402)
...
## PayLabs Web Chat and Public x402 Agent API

PayLabs now supports two entry points:

- **Web Chat**: `/paylabs` continues to use the authenticated Google session and the logged-in user's Circle Developer Controlled Wallet through `/api/paylabs/dcw/run-paid`.
- **Agent API**: external buyer agents call `POST /api/x402/v1/research` with their own x402-compatible wallet and `PAYMENT-SIGNATURE`. PayLabs acts as the seller for the public entry payment and never requires a Google session for this route.

### Public x402 flow

1. Send the research body without a payment header.
2. PayLabs atomically owns the `client_request_id` challenge context and first returns a fixed routing-fee `402 Payment Required` challenge plus a one-time random `read_token`.
3. Sign the routing challenge with the buyer wallet and retry the exact same body with `PAYMENT-SIGNATURE`; only after that settlement does PayLabs run paid Brain preflight and lock the final quote.
4. PayLabs returns a second `402 Payment Required` challenge for the exact locked final execution amount.
5. Sign the final challenge and retry the same request body; PayLabs settles the final entry payment, then executes locked orchestration through trusted server-to-server execution so there is no second internal entry charge or payer-wallet mismatch.

Initial request:

```bash
curl -i https://paylabs.vercel.app/api/x402/v1/research \
  -H 'Content-Type: application/json' \
...
- `GET /api/x402/v1/runs/{runId}/payments`

Discovery metadata is available at `GET /.well-known/x402`, and the OpenAPI 3.1 document is available at `GET /api/x402/v1/openapi.json`.

### TypeScript buyer-agent sketch

```ts
const body = { goal: "Research stablecoin payment agents", route_tier: "auto", max_budget_usdc: "0.01", client_request_id: crypto.randomUUID() };
