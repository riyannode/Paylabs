...
import { DEFAULT_BUDGET_USDC, PUBLIC_API_VERSION, PUBLIC_RESEARCH_PATH, SERVER_MAX_BUDGET_USDC, atomicToUsdc, canonicalJson, constantTimeEqualHex, createReadToken, normalizeAddress, parseUsdc, sha256Hex, usdcToAtomic } from "@/lib/paylabs/public-api/security";
import type { PublicResponseMode, PublicRouteTier } from "@/lib/paylabs/public-api/types";
import { claimExecution, claimPaymentProcessing, publicStatusFromRunStatus, updatePublicRunOrThrow } from "@/lib/paylabs/public-api/lifecycle";
...
export async function POST(req: NextRequest) {
  ...
  if (!signature) {
    ...
    return NextResponse.json({
      ...
      error: { code: "PAYMENT_REQUIRED", message: "A valid x402 payment is required.", retryable: false },
      run_id: run.id,
      retry_url: retryUrl,
  ...
  const lockedResult = await callPaidSeller(createDcwSigner(), {
    ...
    maxAmountUsdc: atomicToUsdc(usdcToAtomic(finalAmountUsdc)),
    requirePayment: true,
  });
