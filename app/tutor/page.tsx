...
export default function TutorPage() {
  ...
  async function proposePath() {
    ...
    try {
      const res = await fetch("/api/paylabs/tutor/propose", {
        ...
        body: JSON.stringify({ goal, budget_usdc: parseFloat(budget) }),
      });
  ...
  async function buyLesson(lessonId: string) {
    setBuyStatus("Buying...");
    try {
      const res = await fetch("/api/paylabs/tutor/buy", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          lesson_id: lessonId,
      ...
      const data = await res.json();
      if (res.ok) {
        setBuyStatus(`Unlocked: ${data.lesson?.title || lessonId}`);
      } else {
        setBuyStatus("Error: " + data.error);
      }
    } catch (e: any) {
      setBuyStatus("Error: " + e.message);
    }
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "grid", gap: "1rem" }}>
          ...
          <button onClick={proposePath} disabled={loading || !goal} className="btn btn-primary">
